...
exports.validateBook = function (data) {
    var errors = {};
    if (data.title) {
        data.title = !isempty_1.isEmpty(data.title)
            ? data.title
    ...
    if (data.author) {
        data.author = !isempty_1.isEmpty(data.author)
            ? data.author
    ...
    if (data.description) {
        data.description = !isempty_1.isEmpty(data.description)
            ? data.description
    ...
    if (data.genre) {
        data.genre = !isempty_1.isEmpty(data.genre)
            ? data.genre
